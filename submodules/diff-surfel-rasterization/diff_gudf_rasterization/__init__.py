...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def backward(ctx, grad_out_color, grad_radii, grad_depth, grad_out_observe):
        ...
        if raster_settings.debug:
            ...
            # torch.save(cpu_args, "./snapshot_bw.dump")
            # print("\nNaN detected in grad_kappas. Writing snapshot_bw.dump for debugging.\n")
            # exit()
        else:
            # cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            # cpu_args = cpu_deep_copy_tuple(args)
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, kappas, shs = None, colors_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
