...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, source_path,
               app_model=None, max_depth=5.0, volume=None, use_depth_filter=False):
    # box
    js_file = f"{source_path}/transforms.json"
    bounds = None
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        gt, _ = view.get_image()
        out = render(view, gaussians, pipeline, background, app_model=app_model, lamda=pipeline.udfw)
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool,
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), scene, gaussians, pipeline, background, dataset.source_path,
